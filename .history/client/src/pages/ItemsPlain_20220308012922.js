...
import {FaIdCard} from 'react-icons/fa';import {FaIdCard} from 'react-icons/fa';
        ...
        {(items || []).length > 0
          ? items.map(item => (
              <ItemContainer key={item._id}>
                ...
                <DetailParagraph>  <FaIdCard/> Exam ID: <Link data-id={item._id} to={`/item/itemPatientExam/${item._id}`}>{item._id}</Link></DetailParagraph>
                <DetailParagraph>Patient ID: {item.PATIENT_ID}</DetailParagraph>
