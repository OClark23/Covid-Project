      ...
      <Wrapper>
        {(items || []).length > 0
          ? items.map(item => ( 
              ...
              <ItemImage src={`https://ohif-hack-diversity-covid.s3.amazonaws.com/covid-png/${items.PATIENT_IMAGES[0].PNG_FILENAME}`}></ItemImage>                 <DetailParagraph>  <FaBookMedical/> Exam ID: <Link data-id={item._id} to={`/item/itemPatientExam/${item._id}`}>{item._id}</Link></DetailParagraph>
