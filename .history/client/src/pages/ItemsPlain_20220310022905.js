      ...
      <Wrapper>
        {(items || []).length > 0 
          ? items.map(item => (
                ...
                {item.PATIENT_IMAGES  ? (
                  <ItemImage src={`https://ohif-hack-diversity-covid.s3.amazonaws.com/covid-png/${item.PATIENT_IMAGES[0].PNG_FILENAME}`}></ItemImage>    
                  <DetailParagraph> ${item.PATIENT_IMAGES[0].EXAM_ID} </DetailParagraph>     
