  ...
  fetchAllPatients = () => {
    api
      ...
      .then(resp => {
        ...
        console.log(items);
        let images = resp.data.PATIENT_IMAGES.map(image => {
          return image.PNG_FILENAME;
        })
        this.setState({ items });
        this.setState({PATIENT_IMAGES: images});
      })
      .catch(err => {
        ...
        {(items || []).length > 0
          ? items.map(item => (
              <ItemContainer key={item._id}>
                <ItemImage src={`https://ohif-hack-diversity-covid.s3.amazonaws.com/covid-png/${item.PATIENT_IMAGES[0]}`}></ItemImage>
