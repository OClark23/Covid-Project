        ...
        {(items || []).length > 0
          ? items.map(item => ( 
              ...
              {items.PATIENT_IMAGES ? > 0
                < ItemImage : } src={`https://ohif-hack-diversity-covid.s3.amazonaws.com/covid-png/${item.PATIENT_IMAGES.PNG_FILENAME}`}></ItemImage>
                <DetailParagraph>  <FaBookMedical /> Exam ID: <Link data-id={item._id} to={`/item/itemPatientExam/${item._id}`}>{item._id}</Link></DetailParagraph><DetailParagraph> <FaIdCard /> Patient ID: <Link data-patient-id={item._id} to={`/item/patient-info/${item._id}`}>{item.PATIENT_ID}</Link>
