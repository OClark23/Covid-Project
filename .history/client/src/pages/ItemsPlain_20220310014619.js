...
class ItemsPlain extends Component {
  ...
  render() {
    ...
    return(
      items.PATIENT_IMAGES(
      <Wrapper>
        {(items || []).length > 0
          ? items.map(item => ( 
              <ItemContainer key={item._id}>
              <ItemImage src={`https://ohif-hack-diversity-covid.s3.amazonaws.com/covid-png/${items.PATIENT_IMAGES.PNG_FILENAME}`}></ItemImage>                 <DetailParagraph>  <FaBookMedical/> Exam ID: <Link data-id={item._id} to={`/item/itemPatientExam/${item._id}`}>{item._id}</Link></DetailParagraph>
